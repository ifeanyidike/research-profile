...
In a real conversation, people don't wait politely for each other to finish. They interrupt, they say "uh-huh" while the other person is still talking, they get distracted by someone else in the room. A voice AI system that can only handle clean turn-taking — where one person speaks, then the other — is not ready for the real world.
...
Whether this is intentional behaviour built into the model or an emergent property of its training data is an open question. But it is a measurable, systematic pattern — and one that has not been reported before for this model.
