...
There is a problem with how TOR is computed. It relies on ASR (automatic speech recognition) to transcribe the model's output and extract word boundary timestamps. The metric then checks whether the last word in the model's transcript ended before the user started speaking. Simple enough. But that means your TOR score depends on two things at once: how well your model behaves, and which transcription system you used to analyze it.
