...
- **Send the whole checklist** each turn. The model was overwhelmed and skipped questions.
...
Vapi exposes a `say` control message that forces the assistant to speak specific text. We tried it.

It worked mechanically: the assistant said the words. But `say` bypasses the model entirely. It's just text-to-speech. There's no intelligence behind it: no acknowledging what the caller just said, no following up when they mention something unexpected, no recovering from a mis-hearing. Even the questions we generated on the backend had no flexibility, since they were static strings, not model output. If the caller said something off-script, the assistant had nothing to work with.
...
4. **Calls the model directly**, not through the platform or any middleware. We pick the model, we set the temperature, we control the full request.

5. **Streams the response back as SSE.** Voice platforms expect Server-Sent Events in OpenAI's chunk format. If you return a single JSON blob instead of a stream, it silently fails and the caller hears nothing.
...
1. **Close-logic gap.** Caller answers a merged question partially ("No scheduling conflicts, but we do have a compliance update..."), and the denied half didn't get marked as answered, so it came back. Fixed in the analyzer's close detection.
